...
import {
  ...
  CheckCircle, AlertCircle, Settings, Info, Layers, Maximize2
} from 'lucide-react';
...
type LeadDetails = { primary?: ConfigRow; secondary?: ConfigRow; [key: string]: unknown };
type ConfigRow = { type?: string; product?: string; width?: number; height?: number; material?: string; steelType?: string; ral?: string; ralCustomCode?: string; paintType?: string; [key: string]: unknown };
type ConfiguratorOptionsValue = {
  steelTypes?: unknown[];
  ralColors?: unknown[];
  paintTypes?: unknown[];
...
export function AdminPanelClient() {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (pendingRaw) {
          try {
            ...
            console.error('Błąd aplikowania pendingu odczytanych:', e instanceof Error ? e.message : e);
          } finally {
  ...
  async function fetchData() {
    try {
      ...
      const antiSpamRow = (s || []).find((row: SettingsRow) => row.key === 'anti_spam_config');
      ...
      const optRow = (s || []).find((row: SettingsRow) => row.key === 'configurator_options');
      if (optRow && optRow.value) {
        const v = optRow.value as ConfiguratorOptionsValue;
        ...
        const stMapped = st
          .map((x: unknown) => {
            ...
            const o = x as { label?: string; price?: number };
            return {
              label: String(o?.label || '').trim(),
              price: Number(o?.price) || 0,
            };
          })
          .filter((x: { label: string }) => x.label && x.label.trim().length >= 2);
        const rcMapped = rc
          .map((x: unknown) => {
            const o = x as { code?: string; name?: string; price?: number };
            return {
          ...
          .filter((x: { code: string }) => x.code.length >= 4);
        const ptMapped = pt
          .map((x: unknown) => {
            const o = x as { label?: string; price?: number };
            return {
          ...
          .filter((x: { label: string }) => x.label.length >= 2);
      ...
      console.error("Błąd pobierania danych:", err instanceof Error ? err.message : err);
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleLocalUpdate = (id: string, field: string, value: string | number) => {
    setHasChanges(true);
  ...
  const handleConfigImageUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await toast.showAlert("Błąd wgrywania (spróbuj pliku bez spacji jeśli ten nie przechodzi): " + (error instanceof Error ? error.message : String(error)));
    } finally {
      setUploading(false);
  ...
  async function saveAllChanges() {
    ...
    try {
      for (const item of prices) {
        /* eslint-disable-next-line @typescript-eslint/no-unused-vars -- omitted from payload */
      ...
      await toast.showAlert("Błąd bazy: " + (error instanceof Error ? error.message : String(error)));
      setSaveStatus('idle');
    }
  }
  ...
  const deletePriceItem = async (id: string, isTemp: boolean) => {
    ...
    try {
      ...
      await toast.showAlert("Błąd usuwania: " + (error instanceof Error ? error.message : String(error)));
    }
  };
  ...
  const handleGalleryImageSelect = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await toast.showAlert("Błąd wgrywania: " + (error instanceof Error ? error.message : String(error)));
    } finally {
      setUploading(false);
      e.target.value = '';
    }
  ...
  const handleAddGalleryItem = async () => {
    ...
    try {
      ...
      await toast.showAlert("Błąd zapisu: " + (error instanceof Error ? error.message : String(error)));
    }
  };

  const updateLead = async (id: number, changes: Record<string, unknown>) => {
    try {
      const { error } = await supabase.from('leads').update(changes).eq('id', id);
      if (error) throw error;
      await fetchData();
    } catch (error: unknown) {
      await toast.showAlert("Błąd aktualizacji zgłoszenia: " + (error instanceof Error ? error.message : String(error)));
    }
  };
  ...
  const saveConfiguratorOptions = async () => {
    ...
    try {
      ...
      await toast.showAlert('Błąd zapisu opcji: ' + (e instanceof Error ? e.message : String(e)));
    } finally {
  ...
  const leadGroups = {
    new: [...leads]
      ...
      .sort((a, b) => {
        const ta = a.created_at ? new Date(a.created_at as string | number).getTime() : 0;
        const tb = b.created_at ? new Date(b.created_at as string | number).getTime() : 0;
        return tb - ta;
      }),
    read: [...leads]
      .filter((lead) => lead.is_read)
  ...
  const flushSeenLeadsToRead = async () => {
    ...
    try {
      ...
      await toast.showAlert("Błąd przenoszenia do odczytanych: " + (error instanceof Error ? error.message : String(error)));
    }
  };
          ...
          {[
            ...
            <button
              ...
              onClick={async () => {
                ...
                setActiveTab(tab.id as 'leads' | 'prices' | 'gallery' | 'options');
              }}
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      await toast.showAlert('Błąd zapisu ustawień: ' + (error instanceof Error ? error.message : String(error)));
                    }
                  }}
                  style={{
                    display: "inline-flex",
              ...
              {leadGroups.new.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 40 }}>
                  {leadGroups.new.map((lead) => {
                    const rawDetails = (lead.details || {}) as LeadDetails;
                      ...
                      >
                        <div style={{ display: "flex", flexDirection: "column", gap: 24, flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            ...
                            {renderConfigRow(primaryDetails, (primaryDetails?.product as string) || "brama")}
                          </div>
              ...
              {leadGroups.read.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 40 }}>
                  {leadGroups.read.map((lead) => {
                    ...
                    const renderConfigRow = (cfg: ConfigRow, label: string) => (
                  ...
                  <div key={item.id} className="admin-card flex flex-col md:flex-row md:items-start" style={{ padding: 28, gap: 28, maxWidth: "100%" }}>
                    ...
                    >
                      {item.image_url ? (
                        ...
                        <img src={item.image_url} alt={item.name ? `Model bramy: ${String(item.name)}` : "Miniatura modelu bramy"} className="w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-all duration-700" />
                      ) : (
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "16px 24px" }}>
                          {[
                            ...
                            <div key={key} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                              ...
                              <input 
                                type="number"
value={String((item as PriceRow)[key] ?? '')}
                                onChange={(e) => handleLocalUpdate(item.id, key, parseInt(e.target.value, 10))}
                                className="admin-input"
                                style={{ width: 100 }}
                              />
                            </div>
                ...
                {prices.filter(p => p.category === 'wicket_base').map((item) => (
                  <div key={item.id} className="admin-card flex flex-col md:flex-row md:items-start" style={{ padding: 28, gap: 28, maxWidth: "100%" }}>
                    ...
                    >
                      {item.image_url ? (
                        /* eslint-disable-next-line @next/next/no-img-element -- dynamic Supabase URL */
                        <img src={item.image_url} alt={item.name ? `Model furtki: ${String(item.name)}` : "Miniatura modelu furtki"} className="w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-all duration-700" />
                      ) : (
                        <div className="h-full flex flex-col items-center justify-center text-zinc-800 group-hover:text-[#D4AF37]">
                          <Camera size={40} />
                          <span className="text-[10px] font-black uppercase mt-3">Wgraj zdjęcie</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6" style={{ gap: 12 }}>
              {gallery.map((item) => (
                <div key={item.id} className="admin-card flex flex-col group overflow-hidden" style={{ padding: 0 }}>
                  ...
                  >
                                        {/* eslint-disable-next-line @next/next/no-img-element -- dynamic Supabase URL */}
                    <img src={item.image_url} alt={item.description ? String(item.description) : "Realizacja"} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-all duration-700" />
                  </button>
